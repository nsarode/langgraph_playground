...
class NoticeEmailExtractor(BaseModel):
...
# OpenAI LLM can't natively extract as date data type
    @staticmethod
    ...
    # Create chain to parse notice emails using NoticeEmailExtractor
...
notice_parser_model = ChatOpenAI(model="gpt-4o-mini", temperature=0, api_key=os.getenv("OPENAI_API_KEY"))

NOTICE_PARSER_CHAIN = (
    info_parse_prompt
    | notice_parser_model.with_structured_output(NoticeEmailExtractor)
)